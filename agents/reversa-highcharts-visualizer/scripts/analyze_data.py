...
def suggest_charts(categories: list, series_analysis: list, n_series: int) -> list:
    ...
    if n_categories <= 20:
        ...
        if n_categories > 8:
            suggestions.append({
                "type": "bar", "score": 85,
                "reason": "Many categories — horizontal bars make label reading easier"
            })
    ...
    if n_series == 1 and n_categories <= 8 and all_positive:
        suggestions.append({
            "type": "pie", "score": 80,
            "reason": "One series with few categories — shows proportion/composition"
        })
