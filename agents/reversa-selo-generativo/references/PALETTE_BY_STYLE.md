...
Table of palettes used by the seal, derived from the visual style chosen by the `/reversa-documentation` user.
...
Cinematic, luxurious, dark style. Focus on contrast and brilliance.
...
Visual translation:
- Background: night-blue black.
- Foreground: gold, wine-red, silver, midnight-blue, smoke-gray.
- Accent: bright gold (brighter than the base gold).
...
**Typical use**: documentation of a system with many components to distinguish; seal covers multiple hues.
...
- Foreground: aurora-pink, glacier-cyan, mist-lilac, soft-yellow, aqua-green.
- Accent: light-white.

**Typical use**: documentation with strong 3D scene presence; the seal dialogues with the `arquitetura.html` aesthetic.

---

## Palette `other` (fallback)

When the user chooses "Other" in the style menu and provides a free-form description, the skill maps the description to the closest palette, or applies a basic heuristic:
...
| Palette position | Visual proportion in seal |
|---|---|
...
For mini-seals (<200px), the palette is simplified to 3 colors only:
...
Maintains readability and visual impact even at reduced size.

---

## Contrast verification

Before rendering, verify that `accent` has sufficient contrast with `bg` for patterns that highlight the center (minimum 4.5:1 ratio per WCAG AA).
