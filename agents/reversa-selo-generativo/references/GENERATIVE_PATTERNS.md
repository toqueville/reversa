...
Catalog of the 5 established patterns that the `selo-generativo` skill produces. Each pattern has a distinct appearance, core algorithm, and parameters derived from the seed.

General seed pattern: the sha256 hash (64 hex chars) is sliced into 8-char segments, each segment becomes a `parseInt(slice, 16)` and feeds a distinct parameter. Thus, different patterns from the same seed share visual personality.

---

## 1. flow-field

Perlin flow fields: thousands of particles follow vectors derived from noise, leaving organic curved trails. Style: "turbulent natural".

**Best with**: `sober` (soft version) and `exploratory` (luminous version) styles.
...
- Field gravity center (if attractor present): XY coordinate (slices 2 and 3).

**Performance**: up to 1500 particles on 800x800 canvas without lag.

---

## 2. particle-orbit

Particles orbiting a center with decaying trails, creating a "rotating constellation" pattern.

**Best with**: `premium` (dark, gold) and `exploratory` (luminous pastels) styles.
...
- Orbit tilt: -pi/4 to pi/4 (slice 1).
...
Symmetric crystalline shape derived from a base polygon, with clean geometric subdivisions. Style: "architectural logo".

**Best with**: `dense` (saturated) and `sober` (clean) styles.
...
Moire-type interference patterns: circular waves emanating from multiple centers that cross each other, generating complex textures from simple rules.

**Best with**: `premium` (black + gold, high contrast) and `dense` styles.
...
Horizontal noise strata, forming an "abstract landscape" with Perlin noise layers.

**Best with**: `sober` (neutral terracotta) and `exploratory` (auroral) styles.
...
- Horizon base height: 25% to 40% of canvas (slice 1).
...
function pickPattern(seedHex, styleHint) {
    ...
    // Soft adjustment by style (choose among "compatible" patterns if there is a mismatch)
...
The `pattern x style` compatibility appears at the beginning of this reference. When there is a declared incompatibility, the `pickCompatible` function re-evaluates among patterns marked as appropriate for the style.

---

## Manual override

The skill accepts a `forcePattern` parameter to ignore the seed-based derivation and choose the pattern manually, useful when the user wants a specific seal in a style different from the default.
