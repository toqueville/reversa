...
Directed dependency graph between modules visualized in 3D, with force simulation (attraction between connected nodes, repulsion between unconnected ones) that distributes the graph in space organically.
...
- Detect **high coupling** (highly connected nodes end up in the center of the cluster).
...
D3-force-type simulation adapted for 3 dimensions. Runs in a loop until stabilized, then freezes.
...
For large graphs (>200 nodes), replace O(n^2) repulsion with **octree (Barnes-Hut)** to reduce to O(n log n).
...
Run Tarjan or Kosaraju before rendering; nodes belonging to cycles receive a special color (orange) and their edges turn red.
...
- **Click on node**: highlights the node and its connected edges, blurs the rest (reduced opacity).
- **Double click on node**: focuses camera on the node.
- **Scroll**: zoom.
...
| 50 to 500 | InstancedMesh + O(n^2) repulsion |
| 500 to 2,000 | InstancedMesh + Barnes-Hut octree |
| > 2,000 | Group by folder first (each cluster = a meta-node) |

Edges: up to **10,000** with LineSegments. Above that, simplify (show only top N by weight) or use color gradient instead of duplicating geometry.
